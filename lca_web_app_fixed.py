...
if st.button("Calculate Impact"):
    ...
    results = {
        ...
        "Water Usage (m3)": round(material_amount * m["water"] + energy_amount * e["water"], 2),
        "Energy Usage (MJ)": round(material_amount * m["energy"] + energy_amount * e["energy"], 2),
        "Acidification (kg SO2 eq)": round(material_amount * m["acid"] + energy_amount * e["acid"], 2)
    }
    ...
    pdf.ln(5)
    for k, v in results.items():
        safe_line = f"{k}: {v}".replace("₂", "2").replace("ₑ", "e").replace("³", "3")
